      ...
      <nav className=" py-2.5 z-10 px-8 pl-12 mt-5 mx-auto container">
        <div className="flex flex-wrap items-center justify-between px-4 w-full h-full">
          <Link to="/" className="flex items-center gap-1">
            ...
            <span className="self-center text-3xl font-medium whitespace-nowrap text-white">Bikesh</span>
          ...
          <div className={` items-center  justify-between w-full lg:flex lg:w-auto lg:order-1 ${isMobileMenuOpen ? '' : 'hidden'}`} id="mobile-menu-2">
            <ul className="flex  flex-col items-center  mt-4 text-sm font-light  lg:flex-row lg:space-x-8 lg:mt-0">
              ...
              <li>
              <Link to="/Work" className={`nav-link ${getLinkClass('/Work')}`}>Work</Link>
              </li>
              <li>
              <Link to="/Services" className={`nav-link ${getLinkClass('/Services')}`}>Services</Link>
              </li>
              <li>
              <Link to="/Resume" className={`nav-link ${getLinkClass('/Resume')}`}>Resume</Link>
              </li>
              <li>
              <Link to="/Contact" className={`nav-link ${getLinkClass('/Contact')}`}>Contact</Link>
              </li>
              <li>
                <Link to="/Hireme" className={` bg-[#1BF7A8]  flex items-center justify-center w-24 h-8 border-0 hover:border hover:border-[#1BF7A8] rounded-xl text-lg ${getLinkClass('/Hireme')}`}>Hire Me</Link>
              </li>
            </ul>
          </div>
        </div>
